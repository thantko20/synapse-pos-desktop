...
export function summarizeVariantUnits(units: VariantUnitLike[]): string {
  ...
  while (true) {
    ...
    const next = children.sort((a, b) => a.factorToParent - b.factorToParent)[0]
  ...
  const defaultLabel = defaultUnit.unitName ?? byId.get(defaultUnit.unitId)?.unitName ?? defaultUnit.unitId
